--TODO:  CHECK TO SEE if exists in current copy of database
-- Checking to see if drop and recreate works.
...
DROP FUNCTION IF EXISTS auditor.set_audit_info(INT, INT);
...
DROP FUNCTION IF EXISTS auditor.get_audit_info();
...
DROP FUNCTION IF EXISTS auditor.clear_audit_info();
...
DROP FUNCTION IF EXISTS auditor.create_auditor_history ( sch TEXT, tbl TEXT );
...
DROP FUNCTION IF EXISTS auditor.create_auditor_func    ( sch TEXT, tbl TEXT );
...
DROP FUNCTION IF EXISTS auditor.create_auditor_lifecycle     ( sch TEXT, tbl TEXT );
...
DROP FUNCTION IF EXISTS auditor.fix_columns();
...
DROP FUNCTION IF EXISTS auditor.update_auditors();
...
END;
$BODY$ LANGUAGE plpgsql;
-- introduces changes to metabib.reingest_metabib_field_entries() that must happen here
