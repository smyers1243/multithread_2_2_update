-- Execute fourth (c) part of aaa trigger on biblio.record_entry.
...
-- Create a wrapper function for the fourth (c) trigger of biblio.record_entry.
-- Must start with CREATE [OR REPLACE] FUNCTION and end with $$ LANGUAGE.
CREATE OR REPLACE FUNCTION biblio.wrap_record_entry_aaa_four_c(start_id BIGINT, end_id BIGINT) 
RETURNS void AS $$
DECLARE
	rec RECORD;
BEGIN
	...
	LOOP
		PERFORM metabib.reingest_metabib_field_entries_search(rec.id);
	END LOOP;
END;
$$ LANGUAGE plpgsql;

-- The SQL that will drop the function
DROP FUNCTION IF EXISTS biblio.wrap_record_entry_aaa_four_c(BIGINT, BIGINT);
...
SELECT biblio.wrap_record_entry_aaa_four_c(~start_id~, ~end_id~);

-- Delete particular records instead of whole table drop
DELETE FROM metabib.keyword_field_entry WHERE field IN ( SELECT id FROM config.metabib_field WHERE name = 'publisher');