-- Execute fourth (a) part of aaa trigger on biblio.record_entry.
...
-- Create a wrapper function for the fourth (a) trigger of biblio.record_entry.
-- Must start with CREATE [OR REPLACE] FUNCTION and end with $$ LANGUAGE.
CREATE OR REPLACE FUNCTION biblio.wrap_record_entry_aaa_four_a(start_id BIGINT, end_id BIGINT) 
RETURNS void AS $$
DECLARE
	rec RECORD;
BEGIN
	...
	LOOP
		PERFORM metabib.reingest_metabib_field_entries_facet(rec.id);
	END LOOP;
END;
$$ LANGUAGE plpgsql;

-- The SQL that will drop the function
DROP FUNCTION IF EXISTS biblio.wrap_record_entry_aaa_four_a(BIGINT, BIGINT);
...
SELECT biblio.wrap_record_entry_aaa_four_a(~start_id~, ~end_id~);